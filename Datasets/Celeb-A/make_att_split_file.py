...
shuffle_identities = np.random.permutation(range(1, n_identities + 1))

# Split
training_classes = shuffle_identities[:math.ceil(n_identities * 0.8)]
validation_classes = shuffle_identities[math.ceil(n_identities * 0.8):math.ceil(n_identities * 0.88)]
test_classes = shuffle_identities[math.ceil(n_identities * 0.88):]
...
for i in training_classes:
    samples = identity_info[i][:-4]
    samples_test_seen = identity_info[i][-4:]
...
with open("att_split.pickle", "wb") as f:
    pickle.dump(dict, f)
