...
class SAE():
    ...
    def __init__(self):
        ...
        with open("pubfig_att_split_50_norm.pickle", "rb") as f:
    ...
    def gzsl_accuracy_semantic(self, weights):
        ...
        print("[INFO]: Test time (GZSL) -> {:.3f}".format(elapsed_time))
    ...
    def test(self, weights):
        zsl_acc, test_time = self.zsl_accuracy_semantic(weights)
        #zsl_feat_acc = self.zsl_accuracy_feature(weights)
        print("The top 1% accuracy [V >>> S] is:", zsl_acc * 100)
        print("[INFO]: Test time (ZSL) -> {:.3f}".format(test_time))
...
if __name__ == "__main__":
    ...
    # Train
    weights, training_time = model.train(lamb)
    print("[INFO]: Training time -> {:.3f}".format(training_time))
    # Test
    model.test(weights)
